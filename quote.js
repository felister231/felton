...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.openMap = function (field) {
    ...
    setTimeout(() => {
      ...
      if (modalMarker && modalMarker.getPosition)
        map.panTo(modalMarker.getPosition());
    }, 250);
  };
  ...
  window.confirmLocation = function () {
    ...
    if (fromCoords && toCoords) drawRouteAndAutoUpdate();
  };
  ...
  function drawRouteAndAutoUpdate() {
    ...
    directionsService.route(req, (result, status) => {
      if (status === "OK" && result) {
        ...
        document.getElementById("totalCost").textContent = `KES ${price.toLocaleString()}`;
